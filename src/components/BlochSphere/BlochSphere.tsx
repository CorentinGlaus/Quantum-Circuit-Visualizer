...
function createValueMesh(x: number, y: number, z: number): THREE.Group {
...
function BlochSphere({
  ...
  useEffect(() => {
    ...
    renderer.setSize(
      (GATE_SIZE - BLOCH_PAD) * CIRCUIT_RATIO,
      (GATE_SIZE - BLOCH_PAD) * CIRCUIT_RATIO,
    );
    ...
    meshGroup.add(
      createAxisLabel("x", new THREE.Vector3(1.7, 0, 0), "#FF0000"),
    );
    meshGroup.add(
      createAxisLabel("y", new THREE.Vector3(0, 1.7, 0), "#00FF00"),
    );
    meshGroup.add(
      createAxisLabel("z", new THREE.Vector3(0, 0, 1.7), "#0000FF"),
    );
  ...
  }, [x, y, z, onCameraChange]);

  useEffect(() => {
    if (!cameraRef.current || !controlsRef.current) return;
    const radius = cameraRef.current.position.length();
    const s = new THREE.Spherical(radius, cameraData.theta, cameraData.phi);
